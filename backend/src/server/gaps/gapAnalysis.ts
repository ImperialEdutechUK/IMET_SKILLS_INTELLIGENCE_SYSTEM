...
/**
 * Store AI-detected employee skills into UserSkill (0–4 scale). Higher observed
 * levels win when the same skill appears across multiple documents.
 */
export async function storeEmployeeSkills(
  ...
  for (const d of detected) {
    ...
    const existing = await prisma.userSkill.findUnique({
      where: { userId_skillId: { userId, skillId: resolved.skill.id } },
    });
    const nextLevel = existing ? Math.max(existing.currentLevel, level) : level;

    await prisma.userSkill.upsert({
      where: { userId_skillId: { userId, skillId: resolved.skill.id } },
      update: { currentLevel: nextLevel },
      create: { userId, skillId: resolved.skill.id, currentLevel: nextLevel, targetLevel: Math.max(nextLevel, 3) },
    });
    stored++;
  }
  return { stored };
